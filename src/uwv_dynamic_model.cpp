...
{
DynamicModel::DynamicModel(int controlOrder, int numberOfThrusters, int numberOfCells, int numberOfVectoring, double samplingTime,
		int simPerCycle,  double initialTime)
: RK4_SIM(controlOrder, (samplingTime/(double)simPerCycle))
{
	underwaterVehicle::DynamicModel::iniatilizeClass(controlOrder, numberOfThrusters, numberOfCells, numberOfVectoring, samplingTime, simPerCycle, initialTime);
}

void DynamicModel::iniatilizeClass(int controlOrder, int numberOfThrusters, int numberOfCells, int numberOfVectoring, double samplingTime,
		int simPerCycle,  double initialTime)
{
	...
	if(!errorConstruction)
	{
		...
		gCellToForce	= Eigen::VectorXd::Zero(2);
		gCellsPositions	= Eigen::MatrixXd::Zero(3,2);
		...
		gNumberOfCells = numberOfCells;
		gNumberOfVectoring = numberOfVectoring;
		gNumberOfThrusters = numberOfThrusters;
	}
}
...
bool DynamicModel::sendRPMCommands(const base::samples::Joints &thrustersInput)
{
	// Checks if the control input is valid
	checkControlInput(thrustersInput, "speed");
	...
	if(!errorStatus)
	{
		Eigen::VectorXd thrustersForce = Eigen::VectorXd::Zero(gNumberOfThrusters);

		// Calculates the forces and moments generated by the thrusters
		rpmToThrustForce(thrustersForce, thrustersInput);
		thrustForceToEffort(gEfforts, thrustersForce);
...
bool DynamicModel::sendCellsCommands(const base::samples::Joints &cellsInput)
{
	// Checks if the control input is valid
	checkControlInput(cellsInput, "cells");
	// Checks if there is any error flag activated
	checkErrors();
	...
	if(!errorStatus)
	{
	  calcCellsEffect(gCellsEfforts,cellsInput, gEulerOrientation);
...
//
bool DynamicModel::sendUpdateStates()
{
  // Checks if there is any error flag activated
	checkErrors();
	...
	if(!errorStatus)
	{
		...
		for (int ii=0; ii < gSimPerCycle; ii++)
			calcStates(systemStates, gCurrentTime, gEfforts-gCellsEfforts);
		...
		const base::Vector6d &controlInput)
{
	...
	acceleration  = invInertiaMatrix * ( gEfforts -gCellsEfforts - coriolisEffect - RBCoriolis - AddedMassCoriolis - LiftEffect  -
...
bool DynamicModel::setUWVParameters(const underwaterVehicle::Parameters &uwvParameters)
{
	...
	if(!errorModelInit)
	{
		if(!errorStatus)
		{
			setInertiaMatrix(uwvParameters.massMatrix, uwvParameters.massMatrixNeg);
			...
			gBuoyancy 		= uwvParameters.waterDensity*uwvParameters.gravity*uwvParameters.uwv_volume;
			...
			gCellToForce		= uwvParameters.cellinput_to_force;
			gCellsPositions		= uwvParameters.cells_positions;
			...
			gNumberOfCells		= uwvParameters.number_of_cells;
			gNumberOfVectoring	= uwvParameters.number_of_vectoring;
			gNumberOfThrusters	= uwvParameters.number_of_Thrusters;
			std::cout << "Vehicle's Acuators: " << std::endl;
			std::cout << "Number of Thrusters: " << gNumberOfThrusters << std::endl;
			std::cout << "Number of Cells: " << gNumberOfCells << std::endl;
			std::cout << "Number of Vectoring Servos: " << gNumberOfVectoring << std::endl;
			return true;
		}
		else
			return false;
...
void DynamicModel::getUWVParameters(underwaterVehicle::Parameters &uwvParameters)
{
	...
	uwvParameters.CorrectionCoefficients  		= gCorrectionCoefficients;
	...
	uwvParameters.cellinput_to_force		= gCellToForce;
	uwvParameters.cells_positions			= gCellsPositions;
	...
	uwvParameters.number_of_cells			= gNumberOfCells;
	uwvParameters.number_of_vectoring		= gNumberOfVectoring;
	uwvParameters.number_of_Thrusters		= gNumberOfThrusters;
	uwvParameters.ctrl_order			= gControlOrder;
	uwvParameters.samplingtime			= gSamplingTime;
}
		...
		const base::Vector6d &velocity)
{
	...
	ModelCorrection(2) 	= 0;
	ModelCorrection(3) 	= 0;
	ModelCorrection(4) 	= gCorrectionCoefficients(0)*u*q;
	ModelCorrection(5) 	= gCorrectionCoefficients(1)*u*r;
}
		...
		const base::Vector3d &eulerOrientation)
{
	...
	gravitybuoyancy(3) 	=  -((yg*gWeight - yb*gBuoyancy)*cos(e2)*cos(e1)) + ((zg*gWeight - zb*gBuoyancy) * cos(e2)*sin(e1));
	gravitybuoyancy(4) 	=   ((zg*gWeight - zb*gBuoyancy)*sin(e2)) + ((xg*gWeight - xb*gBuoyancy)*cos(e2)*cos(e1));
	gravitybuoyancy(5) 	=  -((xg*gWeight - xb*gBuoyancy)*cos(e2)*sin(e1)) - ((yg*gWeight - yb*gBuoyancy)* sin(e2));

}

void DynamicModel::calcCellsEffect(base::Vector6d &cellsEfforts,
		const base::samples::Joints &cellsInput, const base::Vector3d &eulerOrientation)
{
  if(gNumberOfCells == 0) //if the user specify no cells then return zero dive-cells efforts
  {
    cellsEfforts(0)	=  0;
    cellsEfforts(1)	=  0;
    cellsEfforts(2)	=  0;
  ...
  else
  {
    ...
    Eigen::VectorXd J1 = Eigen::VectorXd::Zero(3);
    Eigen::VectorXd cmd  = Eigen::VectorXd::Zero(gNumberOfCells);
    Eigen::VectorXd w  = Eigen::VectorXd::Zero(gNumberOfCells);  //Vector of weights produced by each dive cell
    Eigen::VectorXd Cw = Eigen::VectorXd::Zero(3);
    
    for (int i = 0; i < gNumberOfCells; i++)
    {
      cmd[i] = cellsInput[i].position;
    }
    
    J1 << -sin(e1), cos(e2)*sin(e1), cos(e2)*cos(e1);
    
    w = cmd.array() * gCellToForce.array(); //pairwise multiply the command vector with the cell pos to force transformation to get the force exerted by each cell
    
    Cw = gCellsPositions*w; 

    cellsEfforts(0) 	=   J1[0]*w.sum();
    cellsEfforts(1) 	=   J1[1]*w.sum();
    cellsEfforts(2) 	=   J1[2]*w.sum();
    cellsEfforts(3) 	=  - Cw[1]*J1[2] + Cw[2]*J1[1];
    cellsEfforts(4) 	=  - Cw[2]*J1[0] + Cw[0]*J1[2]; 
    cellsEfforts(5) 	=  - Cw[0]*J1[1] + Cw[1]*J1[0];
  }       
}
...
void DynamicModel::rpmToThrustForce(Eigen::VectorXd &thrustForces,
		const base::samples::Joints &controlInput)
{
	for (int i = 0; i < gNumberOfThrusters; i++)
	{
...
void DynamicModel::setCorrectionCoefficients(const base::VectorXd &CorrectionCoefficients)
{
	gCorrectionCoefficients = CorrectionCoefficients;
}
...
void DynamicModel::checkConstruction(int &controlOrder, double &samplingTime, int &simPerCycle, double &initialTime)
{
...
void DynamicModel::checkParameters(const underwaterVehicle::Parameters &uwvParameters)
{
	...
	else if((int)uwvParameters.thruster_control_matrix.cols() != gNumberOfThrusters
			&& !errorSetParameters)
	{
		LOG_ERROR("\n\n\x1b[31m (Library: uwv_dynamic_model.cpp)"
				...
				, gNumberOfThrusters, uwvParameters.thruster_control_matrix.cols());
		errorSetParameters = true;
	}

	else if((int)uwvParameters.ctrl_order != gNumberOfThrusters + gNumberOfCells + gNumberOfVectoring
			&& !errorSetParameters)
	{
		LOG_ERROR("\n\n\x1b[31m (Library: uwv_dynamic_model.cpp)"
				" The Control order is not equal to the number of thrusters"
				" + number of cells + number of vectoring servos.\x1b[0m\n\n");
		errorSetParameters = true;
		std::cout << "Number of Thrusters: " << gNumberOfThrusters << std::endl;
		std::cout << "Number of Cells: " << gNumberOfCells << std::endl;
		std::cout << "Number of Vectoring Servos: " << gNumberOfVectoring << std::endl;
		...
		std::string jointsElement)
{
	...
	if(!errorStatus)
	{
		...
		else if(jointsElement == "speed" || jointsElement == "raw")
		{
			if (inputSize != gNumberOfThrusters)
			{
				LOG_ERROR("\n\n\x1b[31m (Library: uwv_dynamic_model.cpp)"
						" The system has %i thrusters control inputs, and not %i like it was"
						" provided by the controlInput argument.\x1b[0m\n\n"
						, gNumberOfThrusters, inputSize);
				errorControlInput = true;
			}
		}
		else if(jointsElement == "cells")
		{
		  if (inputSize != gNumberOfCells)
			{
				LOG_ERROR("\n\n\x1b[31m (Library: uwv_dynamic_model.cpp)"
						" The system has %i diving cells control inputs, and not %i like it was"
						" provided by the controlInput argument.\x1b[0m\n\n"
						, gNumberOfCells, inputSize);
				errorControlInput = true;
			}
		}
	}
	...
	if(!errorControlInput && !errorStatus)
	{
		...
		else if(jointsElement == "cells")
		{
			for (uint i = 0; i < controlInput.size(); i++)
			{
				if(!controlInput.elements[i].hasPosition())
				{
					textElement = jointsElement;
					checkError = true;
					break;
...
void DynamicModel::checkThrusterVoltage(const std::vector<double> &thrusterVoltage)
{
	...
	if(thrusterVoltage.size() == gNumberOfThrusters)
	{
		for(int i = 0; i < gNumberOfThrusters; i++)
		{
	...
	else
	{
		if(!errorStatus)
			LOG_ERROR("\n\n\x1b[31m (Library: uwv_dynamic_model.cpp)"
					...
					" \x1b[0m\n\n", gNumberOfThrusters, thrusterVoltage.size());
		errorVoltage = true;
	}
...
void DynamicModel::checkRPMCoefficients(void)
{
	...
	if(gThrusterCoeffRPM.size() != 0 && (int)gThrusterCoeffRPM.size() != gNumberOfThrusters)
	{
	...
	if(sizeError && !errorStatus)
	{
		LOG_ERROR("\n\n\x1b[31m (Library: uwv_dynamic_model.cpp)"
				" The %s has a size equal to %i, but it should have a size"
				" equal to the number of thrusters(%i).\x1b[0m\n\n",
				textElement.c_str(), size, gNumberOfThrusters);
		errorRPMCoeff = true;
	}
